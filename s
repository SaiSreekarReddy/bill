...
REM Prompt for destination server (log can be saved on Server A or B)
set /p LOG_SERVER="Which server should the log be stored on (A/B)? "
...
:LogTransferRemote
REM Function to log file transfer on remote server
...
echo Sending transfer log to %SERVER_LOG% (%SERVER_IP%)...
...
if /i "%LOG_SERVER%"=="A" (
    call :LogTransferRemote "Server A" %SERVER_A% temp_file.txt
) else (
